...
#ESTADO DE LAS VARIABLES DE ECUACIONES
if 'mu' not in st.session_state:
    st.session_state.mu = 0
if 'mu_anterior' not in st.session_state:
    st.session_state.mu_anterior = 0
...
with right_column:
    ...
    if st.button("Aplicar", type="primary", key="btn_aplicar"):
        ...
        st.session_state.mu = monod(nitrogeno,0.5)
    ...
    with sub_col5:
        st.metric(
            label="Chlorella",
            value=f"{st.session_state.mu:.4f}",
            delta="6"
        )
